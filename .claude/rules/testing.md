...
- 레이어별 Fixture 분리: `DomainFixture`(Domain Entity), `JpaFixture`(JPA Entity)
- Fixture 함수는 named parameter + default value 패턴을 사용하여 테스트마다 필요한 값만 오버라이드한다
- Fixture 함수명: `create{EntityName}` (예: `createMember()`, `createMemberJpaEntity()`)
